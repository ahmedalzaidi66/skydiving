...
export default function WishlistHeart({ product, size = 18, onLoginRequired, onToggle, variant = 'card' }: Props) {
  ...
  return (
    <TouchableOpacity style={styles.cardBtn} onPress={handlePress} activeOpacity={0.75} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
      ...
      <Animated.View style={[{ width: 30, height: 30, borderRadius: 15, backgroundColor: Colors.overlay, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: Colors.borderLight, ...(Platform.OS === 'web' ? { backdropFilter: 'blur(6px)' } : {}) }, { transform: [{ scale }] }]}>
        {heartNode}
      </Animated.View>
    </TouchableOpacity>
  );
